...
export default function AdminNotificationsInbox() {
  ...
  return (
    <div>
      ...
      <main className="ml-[252px] p-10 max-w-3xl">
        <header className="flex items-start justify-between gap-4 mb-6 flex-wrap">
          <div>
            ...
            <p className="text-sm text-text-muted mt-1.5">
              Payments, fraud alerts, and other ops events routed to your inbox.
            </p>
          </div>
          {hasUnread && (
            <button
          ...
          {items && items.length === 0 && (
            <div className="p-14 text-center">
              ...
              <p className="text-xs text-text-muted mt-2 max-w-xs mx-auto">
                Fraud alerts and other admin events will land here as they happen.
              </p>
            </div>
          )}
          {items && items.length > 0 && (
            <ul className="divide-y divide-border">
              {items.map((n) => {
                ...
                return (
                  <li key={n.id}>
                    {n.linkUrl ? (
                      <Link href={n.linkUrl} onClick={() => void markOne(n)} className="block">
                        {inner}
                      </Link>
                    ) : (
